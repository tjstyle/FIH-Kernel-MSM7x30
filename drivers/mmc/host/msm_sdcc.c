			...
			if( (fih_get_product_id() == Product_SF6) &&  (fih_get_product_phase() >= Product_PR3))
			{
				...
				if(wimax_on == 1) //if wimax on
				{
					...
					printk(KERN_INFO "%s: Set PMIC gpio 06 output HIGH.\n", __func__);
					   ...
					   printk(KERN_INFO "%s: loop %d start-->  t= [%5lu] \n", __func__,i, (unsigned long) t);
	
			 for(;;)
				 {
							t1 = cpu_clock(printk_cpu);
							nanosec_rem1 = do_div(t1, 1000000000);			   
							printk(KERN_INFO "%s: loop %d -->  t1= [%5lu] \n", __func__,i, (unsigned long) t1);
				   ...
				   if(test_sus==0 )
				   {
								printk(KERN_INFO "%s: Get gpio 142(WiMAX_WAKEUP_HOST_N) = 0\n", __func__);
					 break;
				   }
	
						   if( (t1-t) >= 1 )
						   {
								printk(KERN_INFO "%s:loop %d : Host wait wimax enter ps mode, more than 3 second \n", __func__,i);
			       ...
			       for(;;)
				     {
				       ...
				       if(test_sus==1 )
				       {
								printk(KERN_INFO "%s: Get gpio 142(WiMAX_WAKEUP_HOST_N) = 1\n", __func__);
